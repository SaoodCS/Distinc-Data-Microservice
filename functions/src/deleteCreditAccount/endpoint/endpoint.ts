...
import DelCreditAccountReqBody from '../reqBodyClass/DelCreditAccountReqBody';

export default async function deleteCreditAccount(
   req: express.Request,
   res: express.Response,
): Promise<express.Response> {
   const reqBody = req.body;
   try {
      if (!DelCreditAccountReqBody.isValid(reqBody)) {
      ...
      await CollectionRef.creditAccounts.doc(uid).update({
         [reqBody.id]: FieldValue.delete(),
      });

      return res.status(200).send({ message: 'Successfully Deleted Credit Account' });
   } catch (error: unknown) {
